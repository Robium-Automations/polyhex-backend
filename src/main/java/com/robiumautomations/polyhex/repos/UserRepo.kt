...
interface UserRepo : JpaRepository<User, UserId> {
  ...
  @Query(
      value = "SELECT U.* FROM users U JOIN users_groups UG ON U.user_id = UG.user_id WHERE UG.study_group_id = :studyGroupId AND " +
          "U.university_id = (SELECT U1x.university_id FROM users U1 WHERE U1.user_id = :currentUserId ) ;",
      nativeQuery = true
  )
  ...
  @Query(
      value = "SELECT U.* FROM users U JOIN users_groups UG ON U.user_id = UG.user_id WHERE UG.study_group_id = :studyGroupId AND " +
          "U.university_id = (SELECT U1.university_id FROM users U1 WHERE U1.user_id = :currentUserId ) AND (U.fname ILIKE :nameQuery OR U.lname ILIKE :nameQuery );",
      nativeQuery = true
  )
